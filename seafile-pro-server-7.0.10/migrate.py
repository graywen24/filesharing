...
class ObjMigrateWorker(Thread):
    ...
    def run(self):
        ...
        os.path.getsize(self.object_list_file_path) > 0:
            ...
            for obj in self.dest_store.list_objs():
                if self.invalid_obj(obj):
                    continue
                self.fetch_count += 1
                if obj[0] in self.dest_objs:
                    ...
                    self.dest_objs[obj[0]].add(obj[1])
                if self.object_list_file_path:
                    f.write('/'.join(map(self.to_utf8, obj[:2])) + '\n')
        ...
        if not exists:
            ...
            try: 
                ...
                if self.write_count % 100 == 0:
                    logging.info('[%s] task: %s objects finished.', self.dtype, self.write_count)

                if self.object_list_file_path:
                    with self.lock:
                        self.fd.write('/'.join(map(self.to_utf8, [task.repo_id, task.obj_id])) + '\n')
                        if self.write_count % 100 == 0:
                            self.fd.flush()
            except Exception as e:
