...
#                print "Export filter options: %s" % op.exportfilteroptions
            ...
            outputurl = unohelper.absolutize( self.cwd, unohelper.systemPathToFileUrl(op.output))
            # if not op.stdout:
    ...
    def __init__(self):
        ...
        try:
            ...
            if product.ooName != "LibreOffice" or LooseVersion(product.ooSetupVersion) <= LooseVersion('3.3'):
                self.proc = subprocess.Popen([office.binary, "-invisible", "-nocrashreport", "-nodefault", "-nologo", "-nofirststartwizard", "-norestore", "-accept=%s" % op.connection], env=os.environ)
            else:
                self.proc = subprocess.Popen([office.binary, "--invisible", "--nocrashreport", "--nodefault", "--nologo", "--nofirststartwizard", "--norestore", "--accept=%s" % op.connection], env=os.environ)
        except Exception as e:
            error("Launch of %s failed.\n%s" % (office.binary, e))
        else:
            info(1, "Started %s listener" % product.ooName)
        ...
        if convertor.desktop.getCurrentFrame():
            ...
            try:
                if product.ooName != "LibreOffice" or product.ooSetupVersion <= 3.3:
                    subprocess.Popen([office.binary, "-invisible", "-nocrashreport", "-nodefault", "-nofirststartwizard", "-nologo", "-norestore", "-unaccept=%s" % op.connection], env=os.environ)
                else:
                    subprocess.Popen([office.binary, "--invisible", "--nocrashreport", "--nodefault", "--nofirststartwizard", "--nologo", "--norestore", "--unaccept=%s" % op.connection], env=os.environ)
                ooproc.wait()
...
#        debug_office()
    ...
    from com.sun.star.beans import PropertyValue # pylint: disable=F0401
    from com.sun.star.connection import NoConnectException # pylint: disable=F0401
    from com.sun.star.document.UpdateDocMode import QUIET_UPDATE # pylint: disable=F0401
    from com.sun.star.lang import DisposedException, IllegalArgumentException # pylint: disable=F0401
    from com.sun.star.io import IOException, XOutputStream # pylint: disable=F0401
    from com.sun.star.script import CannotConvertException # pylint: disable=F0401
    from com.sun.star.uno import Exception as UnoException # pylint: disable=F0401
    from com.sun.star.uno import RuntimeException # pylint: disable=F0401
