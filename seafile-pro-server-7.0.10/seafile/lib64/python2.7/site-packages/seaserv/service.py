...
def get_ldap_users(start, offset):
    try:
...
def del_ldap_user(user_id):
    try:
        ret = ccnet_threaded_rpc.del_ldap_user(user_id)
    except SearpcError:
        ret = -1
    return ret

def update_ldap_user(user_id, email, password, is_staff, is_active,
                     extra_attrs=None):
    try:
        ret = ccnet_threaded_rpc.update_ldap_user(user_id, email, password,
                                                  is_staff, is_active, extra_attrs)
    except SearpcError:
        ret = -1
    return ret

def add_ldap_user(email, password, is_staff, is_active, extra_attrs=None):
    try:
        ret = ccnet_threaded_rpc.add_ldap_user(email, password, is_staff,
                                               is_active, extra_attrs)
    except SearpcError:
        ret = -1
    return ret
...
def get_super_users():
    try:
        ret = ccnet_threaded_rpc.get_superusers()
    except SearpcError:
        ret = None
    return ret
...
def create_group(group_name, creator, gtype=None):
    try:
        ret = ccnet_threaded_rpc.create_group(group_name, creator, gtype)
    except SearpcError:
        return -1
    return ret

def group_add_member(group_id, adder, member):
    try:
        ret = ccnet_threaded_rpc.group_add_member(group_id, adder, member)
    except SearpcError:
        return -1
    return ret

def group_remove_member(group_id, adder, member):
    try:
        ret = ccnet_threaded_rpc.group_remove_member(group_id, adder, member)
    except SearpcError:
        return -1
    return ret
...
def get_ldap_groups(start, limit):
    try:
        groups_all = ccnet_threaded_rpc.get_all_groups(start, limit, 'LDAP')
    except SearpcError:
        return None
    return groups_all
...
def remove_group(group_id, user):
    try:
        ret = ccnet_threaded_rpc.remove_group(group_id, user)
    except SearpcError:
        return -1
    return ret

def get_group_dn_pairs():
    try:
        pairs = ccnet_threaded_rpc.get_group_dn_pairs()
    except SearpcError:
        return None
    return pairs

def add_group_dn_pair(group_id, dn):
    try:
        ret = ccnet_threaded_rpc.add_group_dn_pair(group_id, dn)
    except SearpcError:
        return -1
    return ret
...
def create_repo(name, desc, user, passwd, storage_id):
    """
    Return repo id if successfully created a repo, otherwise None.
    """
    try:
        ret = seafserv_threaded_rpc.create_repo(name, desc, user, passwd, storage_id)
    except SearpcError, e:
        logger.error(e)
        ret = None
    return ret
